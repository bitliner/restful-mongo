...
module.exports.collections = function(req, res) {
    var self = this;
    return self.Dao.collections(req.params.host, req.params.db, [], {}, function(err, doc) {
        if (!err && !doc) {
            res.send(404);
        } else if (err) {
...
module.exports.query = function(req, res) {
    ...
    if (req.params.id) {
        return self.Dao.get(req.params.host, req.params.db, req.params.collection, query, fields, options, function(err, doc) {
            if (!err && !doc) {
                res.send(404);
            } else if (err) {
                res.status(500).json(err);
    ...
    self.Dao.query(req.params.host, req.params.db, req.params.collection, query, fields, options, function(err, docs) {
        if (err) {
            res.status(500).json(err);
        } else {
...
module.exports.distinct = function(req, res) {
    ...
    self.Dao.distinct(req.params.host, req.params.db, req.params.collection, query, key, options, function(err, result) {
        if (err) {
            res.json(500, err);
        } else {
            res.json(200, result);
...
module.exports.count = function(req, res) {
    ...
    self.Dao.count(req.params.host, req.params.db, req.params.collection, query, options, function(err, count) {
        if (err) {
            res.json(500, err);
        } else {
