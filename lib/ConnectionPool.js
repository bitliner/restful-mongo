...
ConnectionPool.prototype._getConnectionUrl = function(options) {
    var url = (typeof options.USERNAME !== 'undefined'  ) ? ['mongodb://', options.USERNAME + ':' + options.PASSWORD + '@', options.HOST + ':' + options.PORT, '/' + options.DATABASE_NAME] : ['mongodb://', options.HOST + ':' + options.PORT, '/' + options.DATABASE_NAME]
    url = url.join('')
